...
func generate_small_doner() -> Array[Ingredient]:
	...
	# --- Tomate (0–1) ---
	var tomate_count := randi_range(0, 1)
	for i in tomate_count:
		ingredients.append(tomate)

	# --- Gurke (0–1) ---
	var gurke_count := randi_range(0, 1)
	for i in gurke_count:
		ingredients.append(gurke)
...
func generate_mid_doner() -> Array[Ingredient]:
	...
	# --- Tomate (0–3) ---
	var tomate_count := randi_range(0, 2)
	for i in tomate_count:
		ingredients.append(tomate)

	# --- Gurke (0–3) ---
	var gurke_count := randi_range(0, 2)
	for i in gurke_count:
		ingredients.append(gurke)
...
func generate_big_doner() -> Array[Ingredient]:
	...
	# --- Tomate (1–3) ---
	var tomate_count := randi_range(1, 3)
	for i in tomate_count:
		ingredients.append(tomate)

	# --- Gurke (1–3) ---
	var gurke_count := randi_range(1, 3)
	for i in gurke_count:
		ingredients.append(gurke)

	# --- Salat (0–2) ---
	var salat_count := randi_range(1, 2)
	for i in salat_count:
		ingredients.append(salat)
	...
	# --- Zwiebeln (0–2) ---
	var zwiebel_count := randi_range(0, 2)
	for i in zwiebel_count:
		ingredients.append(zwiebel)
